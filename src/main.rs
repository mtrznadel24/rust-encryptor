...
use file_utils::{
    get_decrypted_filename, get_encrypted_filename, read_encrypted_file, read_key_from_file,
    read_plain_file, write_encrypted_file, write_plain_file,
};
use password::{create_key_from_password, generate_salt, is_password_safe};
use std::io;
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::Decrypt { args } => {
            ...
            let (nonce, ciphertext, salt_opt) =
                read_encrypted_file(&args.file).unwrap_or_else(|e| {
                    eprintln!("Nie udało się odczytać pliku: {}", e);
                    exit(1);
                });
            ...
            let key: Vec<u8> = if let Some(key_path) = &args.key {
                ...
                let salt: [u8; 16] = salt_opt
                    .expect("Brak soli w pliku zaszyfrowanym hasłem")
                    .try_into()
                    .unwrap_or_else(|_| {
